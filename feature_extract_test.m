%Function which returns the feature vectors for the testing images

function [im_test] = feature_extract_test(testimgslocation)


%'im_test' is the resulting matrix of size 7000X769
im_test = zeros(7000,769);
...
for i = 1:14
    ...
    %Reading the testing images from the file location
    temp = imread(strcat(testimgslocation,num2str(i),'.bmp'));
    ...
    im_test(i,:) = horzcat(dwtmat,svdmat);
